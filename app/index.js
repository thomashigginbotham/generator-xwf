...
var XwfGenerator = yeoman.generators.Base.extend({
	...
	askFor: function () {
		...
		var prompts = [{
			name: 'appName',
			message: 'What is the name of your web site/app?'
		}, {
			type: 'confirm',
			name: 'addJasmineKarma',
			message: 'Would you like to add support for Jasmine and Karma?',
			default: true
		}];

		this.prompt(prompts, function (props) {
			this.appName = props.appName;
			this.addJasmineKarma = props.addJasmineKarma;
	...
	app: function () {
		...
		if (this.addJasmineKarma) {
			this.mkdir('test');

			this.copy('test/_sampleSpec.js', 'test/sampleSpec.js');
			this.copy('test/_test-main.js', 'test/test-main.js');
